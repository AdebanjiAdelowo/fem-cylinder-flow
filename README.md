...
The benchmark is the one specified in Schäfer, M. & Turek, S. (1996), *Benchmark Computations of
Laminar Flow Around a Cylinder*, in *Flow Simulation with High-Performance Computers II* (Notes on
Numerical Fluid Mechanics, Vol. 52), Vieweg. The geometry, boundary conditions and quantity
definitions (implemented in `src/geometry.py`, `src/cylinder_bcs.py`, `src/forces.py`) were checked
against the benchmark maintainers' FeatFlow pages; see [`docs/benchmark_audit.md`](docs/benchmark_audit.md).
The comparison intervals in `src/reference.py` are the lower/upper bound rows attributed to Tables 3
and 4 of the 1996 paper; they have been carried since the repository's first commit and were not
re-verified against the paper itself. The higher-precision reference values quoted below (Nabh 1998,
FeatFlow computations) are taken from the FeatFlow benchmark page.
...
![DFG benchmark domain with inflow, wall, cylinder and outflow boundary conditions](docs/figures/domain_bcs.svg)

*Computational domain and boundary conditions, drawn to scale. Boundary labels are the gmsh
physical-group tags used in `src/geometry.py` (1 inlet, 2 outlet, 3 walls, 4 cylinder). The two
orange points are the pressure probes used for $\Delta P$ (`src/forces.py`). The shaded discs show
the region where the gmsh sizing field refines the mesh. Generated by
[`docs/figures/make_domain_figure.py`](docs/figures/make_domain_figure.py), which reads these
...
- **Linear solve**: direct (LU via MUMPS) at every Newton/time step. Correctness and benchmark
  agreement are established first; an iterative Krylov solver with a
...
The solver pipeline, from geometry to benchmark quantities. The manufactured-solution checks on the
left run on the unit square and exercise the same weak form and Newton solver before the cylinder
case is used:

```mermaid
flowchart TD
    MMS["Manufactured solutions on the unit square<br/>A: exact recovery, B: mesh convergence<br/>(scripts/verify_mms.py)"]
    G["gmsh geometry<br/>rectangle minus disk, distance sizing field<br/>geometry order 1 or 2 (src/geometry.py)"]
    S["Taylor-Hood P2/P1 mixed space<br/>(src/spaces.py)"]
    BC["Boundary conditions<br/>no-slip walls and cylinder, parabolic inflow,<br/>do-nothing outflow (src/cylinder_bcs.py)"]
    F["Weak-form residual F(u, p)<br/>steady, or θ-method with θ = 0.5 (Crank-Nicolson)<br/>(src/navier_stokes.py)"]
    N["Newton solve: PETSc SNES, line search<br/>Jacobian J = ufl.derivative(F)<br/>MUMPS direct LU, tolerances 1e-10"]
    D1["2D-1, Re = 20<br/>one steady solve per mesh level"]
    D2["2D-2, Re = 100<br/>time loop, one Newton solve per step"]
    P["Post-processing (src/forces.py)<br/>c_D, c_L with four force definitions<br/>ΔP from two pressure probes"]
    C["Per-cycle analysis (src/periodic.py)<br/>spline extrema, periodicity check,<br/>St from period, FFT and harmonic fit"]
    R["Comparison with reference intervals<br/>and published FeatFlow values"]

    MMS -.same residual and solver.-> F
    G --> S --> BC --> F --> N
    N --> D1 --> P
    N --> D2 --> P
    P --> C --> R
...
mesh-discretisation accuracy).

## Verification: before trusting the cylinder benchmark

The cylinder benchmark is **not** used as the first test of the implementation. `scripts/verify_mms.py` runs two manufactured-solution checks on the unit square
...
<p align="center">
  <img src="figures/mms_convergence.png" width="480"
       alt="Log-log plot of velocity L2, velocity H1 and pressure L2 errors against mesh size, with O(h^3) and O(h^2) reference lines">
</p>

*Check B: errors of the manufactured Navier-Stokes solution against mesh size $h$, with $O(h^3)$
and $O(h^2)$ reference slopes.*
...
defect. The repository contains the corrected test.
...
**At the finest mesh, all three computed quantities fall inside the reference interval**:
$c_D = 5.5744$, $c_L = 0.01043$, $\Delta P = 0.1175$ (`results/2d1_steady_full.txt`). On the coarser
levels (figure: `figures/2d1_convergence_full.png`) $c_D$ increases monotonically toward the
interval (0.46 % and 0.13 % below its lower bound on the first two levels, inside from the third).
$c_L$ and $\Delta P$ do not converge monotonically: $c_L$ is 15.9 % above the upper bound on the
coarsest level, inside on the second, 1.7 % above on the third and inside on the finest; $\Delta P$
is 0.06 % above the interval on the coarsest level and 0.26 % below it on the second, then inside
on the last two. Velocity magnitude (with
...
![2D-1 steady solution: velocity magnitude with streamlines, pressure, and vorticity](figures/2d1_fields_full.png)

*2D-1 steady solution on the finest polygonal mesh (82,913 cells): velocity magnitude with
streamlines, pressure, and vorticity.*

![c_D, c_L and Delta P against degrees of freedom, with the reference interval shaded](figures/2d1_convergence_full.png)

*2D-1 benchmark quantities over the four-level mesh ladder (polygonal geometry, symmetric stress),
with the reference interval shaded. All three quantities lie inside the interval at the finest
level. The approach is not monotone for $c_L$ and $\Delta P$: $\Delta P$ leaves the interval at the
second level before returning.*
...
parameter, so agreement in St is a check on the resolved flow physics rather than on a fitted
quantity.
...
- **2D-3 (the ramped-Reynolds-number case) is not implemented.** Only 2D-1 and 2D-2 are covered; this
  was a scope decision, since the quantities of interest ($c_D$, $c_L$, $\Delta P$, Strouhal number)
  are all exercised by 2D-1 and 2D-2.
- **Direct (MUMPS) linear solves throughout.** Correctness and benchmark agreement were established
  first; an iterative Krylov solver with a fieldsplit (e.g. PCD or SIMPLE-type)
...
- Schäfer, M., Turek, S. (1996). *Benchmark Computations of Laminar Flow Around a Cylinder*, in Flow
  ...
  Vieweg. (Original benchmark specification; the comparison intervals are attributed to its
  Tables 3 and 4, see "Benchmark definition".)
